"""
Implementation of the bias measures as classes
"""
import torch
import pandas as pd
import numpy as np
...
class DemographicParity(Measurement):
    ...
    def calculate_one_relation(self, preds_df, relation):
        ...
        for attr in attribute_set:
            ...
            for target_val in true_target_values:
                ...
                # We note that sum(P(y=t|a) for all t) = 1 and sum(sum(P(y=t | ~a) for all t) = 1
                # P(y=t | a) <= 1 and P(y=t | ~a) <= 1 so their difference is <= 1
                # So sum(|P(y=t|a)p(a) - P(y=t|~a)p(~a)|  for all t) <= 1
    ...
    def calculate(self, evaluator, bias_relations):
        ...
        Return:
        demographic_parity_distance: int
        """
...
class TranslationalLikelihood(Measurement):
    ...
    def update_head_embedding(self, h, r, t_list, score_fn, lr):
        """
        One-step gradient descent update on head entity embedding
    ...
    def calc_bias_on_instr_tail(self, h, new_h_em, instr_rel, instr_t, score_fn):
        ...
        new_em: torch.Tensor, new embeddiing of head entity
    ...
    def calculate_relation(self, dataset, model, target_relation, bias_relation, score_fn=None):
        ...
        model: pykeen.models, an embedding model
        target_relation: str
        bias_relation: str
        """
        self.model = model
